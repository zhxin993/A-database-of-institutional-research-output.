...
export const useItemStore = defineStore("itemStore", {
  state: () => ({
    user: {
      ...
      id: null,
      avatar: null,
      avatar_url: null, // 【新增】在state中增加avatar_url
    },
    userProfile: null, // 【新增】用于存放个人中心信息（包含总获赞数）
  ...
  actions: {
    ...
    async checkAuthStatus() {
      if (this.user.token) {
        try {
          ...
          // 如果token无效或过期，后端会返回错误（例如401）
          // 此时就执行登出操作，清空无效数据
          console.error("Token验证失败，自动登出:", error);
          this.logout();
        }
      }
    },
    login(userData) {
      this.user = {
        ...
        id: userData.id,
        avatar_url: userData.avatar_url, // 保存头像URL
      };
      localStorage.setItem("token", userData.token);
    },

    // 【修改】logout action，清空头像信息
    logout() {
      this.user = { username: "", token: null, id: null, email: null, avatar_url: null };
    ...
    async likeItem(itemId) {
      try {
        ...
        const errorMessage = error.response?.data?.message || '点赞失败，请稍后再试';
        alert(errorMessage);
        throw error;
      }
    },
    ...
    async unlikeItem(itemId) {
      try {
        const response = await api.post(`/items/${itemId}/unlike`);
        return response.data;
      } catch (error) {
